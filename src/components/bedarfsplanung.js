...
export default class DispoEigenfertigung extends Component {
    constructor(props) {
        ...
        this.state = {
            ...
            warehousestock_information: "not defined",
            new_warehousestock: "not defined",
            using_demand: "not defined",
            warehousestock_after_using_demand: "not defined",
            available_amount: "not defined",
            future_movement: "not defined",
            warehousestock_after_movement: "not defined",
            input: "not defined"
        }
...
declare_params() {
        var parameter
        for (parameter of [this.state.n_p1, this.state.n1_p1, this.state.n2_p1, this.state.n3_p1,
        this.state.n_p2, this.state.n1_p2, this.state.n2_p2, this.state.n3_p2,
        this.state.n_p3, this.state.n1_p3, this.state.n2_p3, this.state.n3_p3]) {
            if (parameter == "undefined") { return }
        }

        //Vetriebswunsch definieren (externe Eingabgen)
        var sales_order = { 1: this.state.n_p1, 2: this.state.n_p2, 3: this.state.n_p3 }
        //Prognosen 
        var prognose = {
            2: { 1: this.state.n1_p1, 2: this.state.n1_p2, 3: this.state.n1_p3 },
            3: { 1: this.state.n2_p1, 2: this.state.n2_p2, 3: this.state.n3_p2 },
            4: { 1: this.state.n3_p1, 2: this.state.n2_p3, 3: this.state.n3_p3 }
        }
        //geplante Lagerbestaende definieren (externe Eingaben)
        var planned_p_stock = { 1: 0, 2: 0, 3: 0 };
        var planned_e_stock = { 1: this.state.n1_p1, 2: this.state.n1_p2, 3: this.state.n1_p3 };
...
var waiting_list = this.get_waitinglist_by_producing_items(this.state.xml)
    ...
    working_hours_planning(dispo, xml) {
        ...
        for (workstation of Object.keys(total_capacity_plan)) {
            ...
            schichten= 1
            if (working_hours > 240) { working_hours = 240 }
            //2-Schichten Betrieb
            if (total_capacity_plan[workstation] >= 4200 && working_hours > 240) {
                working_hours = parseInt((total_capacity_plan[workstation] - 2400 * 2) / 5)
                schichten= 2
                if (working_hours > 240) { working_hours = 240 }
                //3-Schichten Betrieb
                if (total_capacity_plan[workstation] >= 6600 && working_hours > 240) {
                    working_hours = 0
    ...
    create_xml(params, orders, production, hours) {
        ...
        var productionlist = doc.createElement("productionlist");
        var order
        for (order of Object.keys(production)) {
            var item = doc.createElement("production");
            item.setAttribute("article", order.toString());
            if (production[order] <= 0) {
                continue
        ...
        for (order_i of Object.keys(orders)) {
            var order
            for (order of Object.keys(orders[order_i])) {
                var item = doc.createElement("order");
                item.setAttribute("article", order.toString());
                item.setAttribute("quantity", orders[order_i][order]['menge'].toString());
                if (orders[order_i][order]['bestellart'] == "Eil") { var art = 4 } else { var art = 5 }
                item.setAttribute("modus", art.toString());
                orderlist.appendChild(item)
            }
        }
    ...
    handle_submit = (event) => {
        ...
        ///////////////////////////////////////////////
        //3. Dispositionsprogramm berechnen
        // Wichtig: Ausgabe von dispo[item]['production_order'] und ggf. Weitergabe als state
        ...
        /////////////////////////////////////////////
        //4. Kapazitätsplanung berechnen
        // Wichtig: Ausgabe von working_hours[workplace]['Überstunden'],
        // working_hours[workplace]['Schichten'] und ggf. Weitergabe als state
        ...
        var waiting_list = this.get_waitinglist_by_purchase_items(this.state.xml);
        console.log("Kaufteile aus Warteliste", waiting_list)
        var warehousestock = this.get_warehousestock_by_purchase_items(this.state.xml);
        ...
        var merge_periods = this.merge_periods(production_order, this.state.params['prognose']);
        ...
        var future_movement = this.calculate_future_movement(this.state.xml, ranges)
        ...
        var working_hours = this.working_hours_planning(new_dispo, this.state.xml)
        console.log("Kapazitätsprogramm", working_hours)
 ...
 //xml output
        ...
        var input = this.create_xml(this.state.params, erfasste_Lieferungen, production, working_hours)
        var input = this.create_xml(this.state.params, erfasste_Lieferungen, production, working_hours)
        console.log(typeof (this.state.n_p1))
    }
    ...
    render() {
        return (<form onSubmit={this.handle_submit}>
            P1:
            <input type="number" name="n_p1" step="50" value={this.state.n_p1} onChange={this.handleChange} />
            <input type="number" name="n1_p1" step="50" value={this.state.n1_p1} onChange={this.handleChange} />
            <input type="number" name="n2_p1" step="50" value={this.state.n2_p1} onChange={this.handleChange} />
            <input type="number" name="n3_p1" step="50" value={this.state.n3_p1} onChange={this.handleChange} />
            <br />
            P2:
            <input type="number" name="n_p2" step="50" value={this.state.n_p2} onChange={this.handleChange} />
            <input type="number" name="n1_p2" step="50" value={this.state.n1_p2} onChange={this.handleChange} />
            <input type="number" name="n2_p2" step="50" value={this.state.n2_p2} onChange={this.handleChange} />
            <input type="number" name="n3_p2" step="50" value={this.state.n3_p2} onChange={this.handleChange} />
            <br />
            P3:
            <input type="number" name="n_p3" step="50" value={this.state.n_p3} onChange={this.handleChange} />
            <input type="number" name="n1_p3" step="50" value={this.state.n1_p3} onChange={this.handleChange} />
            <input type="number" name="n2_p3" step="50" value={this.state.n2_p3} onChange={this.handleChange} />
            <input type="number" name="n3_p3" step="50" value={this.state.n3_p3} onChange={this.handleChange} />
            <br />
            ...
            <h1> Kaufteile aus Warteschlangen (abhängig von Erzeugnisse): </h1>
            {JSON.stringify(this.state.waiting_purchase_items)}
            <h1> Kaufteile aus Produktionsaufträgen (abhängig von Erzeugnisse): </h1>
            {JSON.stringify(this.state.production_order_purchase_items)}
            <h1> Lagerbestand_informationen </h1>
            {JSON.stringify(this.state.warehousestock_information)}
            <h1> Lagerbestand(neu) </h1>
            {JSON.stringify(this.state.new_warehousestock)}
            <h1> Bedarf(Gesamt)</h1>
            {JSON.stringify(this.state.using_demand)}
            <h1> Lagerbestand(abzüglich Bedarf)</h1>
            {JSON.stringify(this.state.warehousestock_after_using_demand)}
            <h1> vorhandene Menge (%)</h1>
            {JSON.stringify(this.state.available_amount)}
            <h1> ausstehende Lieferungen</h1>
            {JSON.stringify(this.state.future_movement)}
            <h1> Lagerbestand plus Lieferungen</h1>
            {JSON.stringify(this.state.warehousestock_after_movement)}
            <h1>Magic Download link</h1>
            <a href={this.state.input} download="input.xml">Donwload</a>
