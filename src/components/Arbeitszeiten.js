...
class Arbeitszeiten extends Component {
    ...
    // handleChange = (evt) => {
    //     const { handleChangeInContainer } = this.props;
    //     handleChangeInContainer(evt, 'arbeitszeiten');
	// }
    ...
    handleChange = (evt) => {
		...
		let max_value = 240;
        // arbeitszeiten[evt.target.name] = parseInt(evt.target.value);
        // console.log('lolo');
		...
		if (!isNaN(evt.target.value)) {
			Object.keys(arbeitszeiten).map( (arbeitsstelle, index) => {
				Object.keys(arbeitszeiten[arbeitsstelle]).map( key => {
					...
					// {arbeitszeiten[1].überstunden_gesamt1}
								...
								<td style={{padding:'10px 0px 0px 15px'}}>
									...
									<table style={{backgroundColor:'#c1c1bf',margin:'auto'}}>
										...
										{/* <tr>
									   		<td>
								   				5
										     	<input autoComplete='off' type="hidden" name="wplace4" value="5"/>
										   </td>
   											<td>

   												<input autoComplete='off' readOnly="" style={{backgroundColor:'#d1d1d1'}} size="4" name="shift4" value="0" maxLength="5" type="text" onChange={this.handleChange} />

											</td>
   											<td>

   												<input autoComplete='off' readOnly="" style={{backgroundColor:'#d1d1d1'}} size="4" name="overtime4" value="0" maxLength="5" type="text" onChange={this.handleChange} />
